...
/// 关键属性：
///   - id_Base_one: 分类唯一标识（与 TitleModel 的 titleCategory_Base_one 对应）
///   - name_Base_one: 展示名称（英文 UI）
///   - iconName_Base_one: SF Symbol 图标名
///   - colorHex_Base_one: 分类主题色十六进制值
struct HomeCategory_Base_one {
    
    /// 分类唯一标识（"all" 表示全部）
    let id_Base_one: String
    
    /// 分类展示名称
    let name_Base_one: String
    
    /// SF Symbol 图标名
    let iconName_Base_one: String
    
    /// 分类主题色十六进制值
    let colorHex_Base_one: String
}
...
class TitleModel_Base_one: NSObject, Codable {
    ...
    /// 帖子所属家居分类标识（与 HomeCategory_Base_one.id_Base_one 对应）
    var titleCategory_Base_one: String
    ...
    init(titleId_Base_one: Int,
         ...
         likes_Base_one: Int,
         titleCategory_Base_one: String = "all") {
        ...
        self.titleCategory_Base_one = titleCategory_Base_one
    }
