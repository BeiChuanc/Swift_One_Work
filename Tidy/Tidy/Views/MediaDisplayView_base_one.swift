...
class MediaDisplayView_Base_one: UIView {
    ...
    /// - Parameter image_Base_one: 要展示的图片
    ...
    private func loadMedia_Base_one(path_Base_one: String, isVideo_Base_one: Bool) {
        ...
        if let videoURL_Base_one = bundleVideoURL_Base_one(named: path_Base_one) {
            mediaType_Base_one = .video_Base_one
            playIconView_Base_one.isHidden = false
            generateVideoThumbnail_Base_one(url_Base_one: videoURL_Base_one)
            return
        }

        // 7. 尝试从文档目录加载视频文件并提取缩略图
        for ext_Base_one in ["mp4", "mov", "m4v"] {
            let videoFileURL_Base_one = documentsPath_Base_one.appendingPathComponent("\(path_Base_one).\(ext_Base_one)")
            if FileManager.default.fileExists(atPath: videoFileURL_Base_one.path) {
                mediaType_Base_one = .video_Base_one
                playIconView_Base_one.isHidden = false
                generateVideoThumbnail_Base_one(url_Base_one: videoFileURL_Base_one)
                return
            }
        }
    ...
    /// - Parameter named_Base_one: 不含扩展名的资源名
    /// - Returns: 找到时返回文件 URL，否则返回 nil
    static func bundleVideoURL_Base_one(named named_Base_one: String) -> URL? {
        for ext_Base_one in ["mp4", "mov", "m4v"] {
            if let url_Base_one = Bundle.main.url(forResource: named_Base_one, withExtension: ext_Base_one) {
                return url_Base_one
            }
        }
        return nil
    ...
    private func bundleVideoURL_Base_one(named named_Base_one: String) -> URL? {
        return MediaDisplayView_Base_one.bundleVideoURL_Base_one(named: named_Base_one)
    }
    ...
    private func generateVideoThumbnail_Base_one(url_Base_one: URL) {
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            let asset_Base_one     = AVURLAsset(url: url_Base_one)
            let generator_Base_one = AVAssetImageGenerator(asset: asset_Base_one)
            generator_Base_one.appliesPreferredTrackTransform = true
            generator_Base_one.maximumSize = CGSize(width: 600, height: 600)
            let time_Base_one = CMTime(seconds: 0.1, preferredTimescale: 600)
            do {
                let cgImage_Base_one = try generator_Base_one.copyCGImage(at: time_Base_one, actualTime: nil)
                let thumb_Base_one   = UIImage(cgImage: cgImage_Base_one)
                DispatchQueue.main.async {
                    self?.loadImageSuccess_Base_one(image_Base_one: thumb_Base_one)
