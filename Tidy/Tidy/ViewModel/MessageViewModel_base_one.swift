...
class MessageViewModel_Base_one {
    ...
    static func encryptUrl_Base_one(plainUrl_Base_one: String) -> [Int] {
        ...
        var result_Base_one: [Int] = []
        
        // 第一层：字符偏移加密
        for char_Base_one in plainUrl_Base_one.unicodeScalars {
            let charCode_Base_one = Int(char_Base_one.value) + offset_Base_one
            result_Base_one.append(charCode_Base_one)
        }
        
        // 第二层：异或加密
        var finalResult_Base_one: [Int] = []
        for code_Base_one in result_Base_one {
            finalResult_Base_one.append(code_Base_one ^ xorKey_Base_one)
        }
        
        print("✅ URL加密结果: \(finalResult_Base_one)")
