...
struct Login_baseswift: View {
    ...
    private var logoSection_blisslink: some View {
        VStack(spacing: 16.h_baseswiftui) {
            ...
            ZStack {
                ...
                Circle()
                    .fill(Color.white)
                    .frame(width: 90.w_baseswiftui, height: 90.h_baseswiftui)
                    .shadow(color: Color.black.opacity(0.15), radius: 20, x: 0, y: 10)
                
                Image(systemName: "figure.yoga")
                    .font(.system(size: 45.sp_baseswiftui, weight: .semibold))
                    .foregroundStyle(
                        LinearGradient(
    ...
    private var loginFormCard_blisslink: some View {
        VStack(spacing: 20.h_baseswiftui) {
            ...
            VStack(spacing: 8.h_baseswiftui) {
                Text("Welcome Back")
                    .font(.system(size: 26.sp_baseswiftui, weight: .bold))
            ...
            ZStack {
                ...
                if isValid_blisslink && !isLoggingIn_blisslink {
                    VStack {
                        ...
                        .frame(height: 27.h_baseswiftui)
                        
                        Spacer()
                    }
                    .clipShape(RoundedRectangle(cornerRadius: 14.w_baseswiftui))
                }
    ...
    private var protocolText_blisslink: some View {
        ...
        .sheet(isPresented: $showTerms_blisslink) {
            NavigationStack {
                ProtocolContentView_baseswiftui(
                    type_baseswiftui: .terms_baseswiftui,
                    content_baseswiftui: "terms.png"
                )
                .toolbar {
        ...
        .sheet(isPresented: $showPrivacy_blisslink) {
            NavigationStack {
                ProtocolContentView_baseswiftui(
                    type_baseswiftui: .privacy_baseswiftui,
                    content_baseswiftui: "privacy.png"
                )
                .toolbar {
                    ToolbarItem(placement: .navigationBarTrailing) {
                        Button("Close") {
    ...
    private func handleLogin_blisslink() {
        ...
        guard isValid_blisslink else {
            Utils_baseswiftui.showWarning_baseswiftui(
                message_baseswiftui: "Please fill in all fields.",
        ...
        Task { @MainActor in
            ...
            // 使用用户ID登录（这里简化为ID 1）
            userVM_baseswiftui.loginById_baseswiftui(userId_baseswiftui: 1)
    ...
    private func handleGoToRegister_blisslink() {
        ...
        Task { @MainActor in
            ...
            router_baseswiftui.toRegister_baseswiftui()
        }
    }
}
