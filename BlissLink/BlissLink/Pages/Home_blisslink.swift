...
struct Home_baseswift: View {
                ...
                VStack {
                    ...
                    BackgroundSelector_blisslink(
                        selectedBackground_blisslink: Binding(
                            get: { userVM_blisslink.getCurrentYogaMatBackground_blisslink() },
                            set: { userVM_blisslink.changeYogaMatBackground_blisslink(background_blisslink: $0) }
                        ),
                    ...
                    .sheet(isPresented: $showAddMemory_blisslink) {
                        AddMemorySheet_blisslink(
                            ...
                            onAdd_blisslink: { sticker_blisslink in
                                userVM_blisslink.addMemorySticker_blisslink(sticker_blisslink: sticker_blisslink)
                            },
                            existingStickers_blisslink: userVM_blisslink.getMemoryStickers_blisslink()
                        )
                    }
            }
    ...
    private var yogaMatBackground_blisslink: some View {
        LinearGradient(
            gradient: Gradient(colors: userVM_blisslink.getCurrentYogaMatBackground_blisslink().gradientColors_blisslink),
            startPoint: .topLeading,
            endPoint: .bottomTrailing
        )
        .overlay(
            // 图案装饰
            Image(systemName: userVM_blisslink.getCurrentYogaMatBackground_blisslink().iconName_blisslink)
                .font(.system(size: 200.sp_blisslink, weight: .thin))
                .foregroundColor(.white.opacity(0.08))
                .offset(x: 50.w_blisslink, y: -100.h_blisslink)
        )
    }
    ...
    private var topControlBar_blisslink: some View {
        HStack {
            ...
            VStack(alignment: .leading, spacing: 4.h_blisslink) {
                ...
                Text(userVM_blisslink.getCurrentUser_blisslink().userName_blisslink ?? "Guest")
    ...
    private var yogaMatMainArea_blisslink: some View {
        let hasStickers_blisslink = !userVM_blisslink.getMemoryStickers_blisslink().isEmpty
        let matHeight_blisslink = hasStickers_blisslink ? APPSCREEN_baseswift.WIDTH_baseswift * 1.3 : 150.h_blisslink
        ...
        return GeometryReader { geometry_blisslink in
            ZStack {
                ...
                RoundedRectangle(cornerRadius: 24.w_blisslink)
                    ...
                    .blur(radius: 30)
                    .frame(height: hasStickers_blisslink ? geometry_blisslink.size.width * 1.3 : 150.h_blisslink)
                
                // 纪念贴纸层
                ForEach(userVM_blisslink.getMemoryStickers_blisslink()) { sticker_blisslink in
                ...
                // 添加纪念贴纸按钮
                if userVM_blisslink.getMemoryStickers_blisslink().count < 5 {
                    VStack {
                        Spacer()
                        HStack {
                            Spacer()
    ...
    private var badgesSection_blisslink: some View {
        VStack(alignment: .leading, spacing: 12.h_blisslink) {
            HStack {
                ...
                Text("\(getUnlockedBadgeCount_blisslink())/\(localData_blisslink.badgeList_blisslink.count)")
                    .font(.system(size: 14.sp_blisslink, weight: .semibold))
                    .foregroundColor(currentBackground_blisslink.secondaryTextColor_blisslink)
            }
            .padding(.horizontal, 20.w_blisslink)
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 16.w_blisslink) {
                    ForEach(Array(localData_blisslink.badgeList_blisslink.prefix(5).enumerated()), id: \.element.id) { index_blisslink, badge_blisslink in
                        BadgeView_blisslink(
                            badge_blisslink: badge_blisslink,
                            size_blisslink: 60.w_blisslink,
    ...
    private var friendActivitiesSection_blisslink: some View {
        VStack(alignment: .leading, spacing: 12.h_blisslink) {
            ...
            VStack(spacing: 12.h_blisslink) {
                ForEach(Array(localData_blisslink.friendActivities_blisslink.prefix(3).enumerated()), id: \.element.id) { index_blisslink, activity_blisslink in
    ...
    private var currentBackground_blisslink: YogaMatBackground_blisslink {
        return userVM_blisslink.getCurrentYogaMatBackground_blisslink()
    }
    ...
    private func getUnlockedBadgeCount_blisslink() -> Int {
        return localData_blisslink.badgeList_blisslink.filter { $0.isUnlocked_blisslink }.count
    }
    ...
    private func handleFriendTap_blisslink(_ activity_blisslink: FriendActivity_blisslink) {
        ...
        // 找到对应的好友信息
        if let friend_blisslink = localData_blisslink.userList_blisslink.first(where: { $0.userId_blisslink == activity_blisslink.friendUserId_blisslink }) {
            // 跳转到好友信息页（串门）
            router_blisslink.toUserInfo_blisslink(user_blisslink: friend_blisslink)
        }
    ...
    private func handleTimerTap_blisslink() {
        ...
        // 跳转到计时页面
        router_blisslink.navigate_blisslink(to: .practiceTimer_blisslink)
    ...
    private func handleDeleteMemory_blisslink(_ sticker_blisslink: MemorySticker_blisslink) {
        ...
        // 从用户数据中移除
        userVM_blisslink.deleteMemorySticker_blisslink(sticker_blisslink: sticker_blisslink)
