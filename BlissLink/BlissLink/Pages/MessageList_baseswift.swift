...
struct MessageList_baseswift: View {
        ...
        ZStack {
            ...
            ZStack {
                ...
                Circle()
                    ...
                    .offset(x: 170.w_baseswiftui, y: 250.h_baseswiftui)
                    .blur(radius: 55)
                
                Circle()
                    .fill(Color(hex: "FA8BFF").opacity(0.05))
                    .frame(width: 290.w_baseswiftui, height: 290.h_baseswiftui)
                    .offset(x: -90.w_baseswiftui, y: 550.h_baseswiftui)
                    .blur(radius: 48)
                ...
                VStack(spacing: 12.h_baseswiftui) {
                    ForEach(chatUsers_blisslink.indices, id: \.self) { index_blisslink in
                        ...
                        ChatUserCard_blisslink(
                            user_blisslink: user_blisslink,
                            lastMessage_blisslink: messageVM_baseswiftui.getLastMessageWithUser_baseswiftui(userId_baseswiftui: user_blisslink.userId_baseswiftui ?? 0),
                            onTap_blisslink: {
                                handleUserTap_blisslink(user_blisslink)
                            }
                        )
    ...
    /// 有聊天记录的用户
    private var chatUsers_blisslink: [PrewUserModel_baseswiftui] {
        return messageVM_baseswiftui.getChatUsers_baseswiftui()
    }
    
    /// 推荐用户（排除已聊天的）
    private var recommendedUsers_blisslink: [PrewUserModel_baseswiftui] {
        let chatUserIds_blisslink = chatUsers_blisslink.compactMap { $0.userId_baseswiftui }
        return localData_baseswiftui.userList_baseswiftui.filter { user_blisslink in
            guard let userId_blisslink = user_blisslink.userId_baseswiftui else { return false }
    ...
    /// 处理用户点击
    private func handleUserTap_blisslink(_ user_blisslink: PrewUserModel_baseswiftui) {
        // 触觉反馈
        let generator_blisslink = UIImpactFeedbackGenerator(style: .light)
        generator_blisslink.impactOccurred()
        
        // 跳转到聊天页面
        router_baseswiftui.toUserChat_baseswiftui(user_baseswiftui: user_blisslink)
    }
}
            ...
            VStack(spacing: 12.h_baseswiftui) {
                ...
                ZStack {
                    ...
                    Circle()
                        ...
                        .frame(width: 82.w_baseswiftui, height: 82.h_baseswiftui)
                        .blur(radius: 8)
                    ...
                    UserAvatarView_baseswiftui(
                        ...
                        size_baseswiftui: 74.w_baseswiftui,
                    ...
                    Circle()
                        .fill(Color.green)
                        .frame(width: 16.w_baseswiftui, height: 16.h_baseswiftui)
                        .overlay(
                            Circle()
                                .stroke(Color.white, lineWidth: 3.w_baseswiftui)
                        )
                        .offset(x: 26.w_baseswiftui, y: 26.h_baseswiftui)
                }
                
                // 用户名
                VStack(spacing: 4.h_baseswiftui) {
                    ...
                    // 关注者数量
                    if let followers_blisslink = user_blisslink.userFans_baseswiftui {
                        Text("\(followers_blisslink) followers")
                            .font(.system(size: 11.sp_baseswiftui))
                            .foregroundColor(.secondary)
                    }
                }
                .frame(width: 85.w_baseswiftui)
            }
...
struct ChatUserCard_blisslink: View {
    ...
    let user_blisslink: PrewUserModel_baseswiftui
    let lastMessage_blisslink: MessageModel_baseswiftui?
    var onTap_blisslink: (() -> Void)?
            ...
            HStack(spacing: 16.w_baseswiftui) {
                ...
                ZStack {
                    ...
                    Circle()
                        ...
                        .frame(width: 64.w_baseswiftui, height: 64.h_baseswiftui)
                        .blur(radius: 6)
                    ...
                    UserAvatarView_baseswiftui(
                        ...
                        avatarPath_baseswiftui: user_blisslink.userHead_baseswiftui,
                        userName_baseswiftui: user_blisslink.userName_baseswiftui,
                        size_baseswiftui: 58.w_baseswiftui
                    )
                    
                    // 在线状态
                    Circle()
                        .fill(Color.green)
                        .frame(width: 14.w_baseswiftui, height: 14.h_baseswiftui)
                        .overlay(
                            Circle()
                                .stroke(Color.white, lineWidth: 2.5.w_baseswiftui)
                        )
                        .offset(x: 20.w_baseswiftui, y: 20.h_baseswiftui)
                }
                ...
                VStack(alignment: .leading, spacing: 6.h_baseswiftui) {
                    ...
                    HStack(spacing: 6.w_baseswiftui) {
                        if let lastMessage_blisslink = lastMessage_blisslink {
                            if lastMessage_blisslink.isMine_baseswiftui == true {
                                Text("You:")
                                    .font(.system(size: 13.sp_baseswiftui, weight: .semibold))
                                    .foregroundColor(.secondary)
                            }
                            
                            Text(lastMessage_blisslink.content_baseswiftui ?? "")
                                .font(.system(size: 14.sp_baseswiftui))
                                .foregroundColor(.secondary)
                                .lineLimit(2)
                        } else {
                ...
                // 右侧信息
                VStack(alignment: .trailing, spacing: 10.h_baseswiftui) {
                    // 时间
                    if let lastMessage_blisslink = lastMessage_blisslink,
                       let time_blisslink = lastMessage_blisslink.time_baseswiftui {
                        Text(time_blisslink)
                    ...
                    if lastMessage_blisslink != nil {
                        ZStack {
                            Circle()
                                .fill(Color.red)
                                .frame(width: 20.w_baseswiftui, height: 20.h_baseswiftui)
                            
                            Text("2")
                                .font(.system(size: 11.sp_baseswiftui, weight: .bold))
            ...
            .background(
                ZStack {
                    ...
                    HStack {
                        ...
                        .frame(width: 4.w_baseswiftui)
                        .cornerRadius(2.w_baseswiftui)
                        
                        Spacer()
                    }
                    .padding(.leading, 4.w_baseswiftui)
                }
