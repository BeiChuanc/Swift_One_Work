...
struct MediaPlayer_baseswift: View {
        ...
        if let videoURL_blisslink = getVideoURL_blisslink() {
            ...
            VStack(spacing: 20.h_blisslink) {
                ...
                Text("Video not found")
                    .font(.system(size: 18.sp_blisslink, weight: .medium))
    ...
    private func getVideoURL_blisslink() -> URL? {
        // 1. 检查是否是网络视频
        if mediaUrl_blisslink.hasPrefix("http://") || mediaUrl_blisslink.hasPrefix("https://") {
            return URL(string: mediaUrl_blisslink)
        }
        
        // 2. 检查是否是Bundle中的视频
        let fileName_blisslink: String
        if mediaUrl_blisslink.hasSuffix(".mp4") {
            fileName_blisslink = mediaUrl_blisslink.replacingOccurrences(of: ".mp4", with: "")
        } else {
            fileName_blisslink = mediaUrl_blisslink
        }
        ...
        if let documentsDirectory_blisslink = fileManager_blisslink.urls(for: .documentDirectory, in: .userDomainMask).first {
            let fileURL_blisslink = documentsDirectory_blisslink.appendingPathComponent("\(mediaUrl_blisslink).mp4")
            if fileManager_blisslink.fileExists(atPath: fileURL_blisslink.path) {
                return fileURL_blisslink
            }
        }
        
        print("⚠️ 无法找到视频：\(mediaUrl_blisslink)")
        return nil
    }
    ...
    private func loadImage_blisslink() -> UIImage? {
        // 1. 检查是否是系统图标
        if let systemImage_blisslink = UIImage(systemName: mediaUrl_blisslink) {
            // 将系统图标转换为带背景的图片
            return createImageFromSystemIcon_blisslink(iconName: mediaUrl_blisslink)
        }
        
        // 2. 检查是否是网络图片（这里只做标记，实际需要异步加载）
        if mediaUrl_blisslink.hasPrefix("http://") || mediaUrl_blisslink.hasPrefix("https://") {
            // 注意：这里需要异步加载，暂时返回nil
            print("⚠️ 网络图片需要异步加载：\(mediaUrl_blisslink)")
            return nil
        }
        
        // 3. 尝试从 Assets 加载
        if let image_blisslink = UIImage(named: mediaUrl_blisslink) {
        ...
        print("⚠️ 无法加载图片：\(mediaUrl_blisslink)")
        return nil
    }
    ...
    private func loadImageFromDocuments_blisslink() -> UIImage? {
        ...
        // 尝试带 .jpg 扩展名
        var fileURL_blisslink = documentsDirectory_blisslink.appendingPathComponent("\(mediaUrl_blisslink).jpg")
        if let image_blisslink = UIImage(contentsOfFile: fileURL_blisslink.path) {
            return image_blisslink
        }
        
        // 尝试不带扩展名
        fileURL_blisslink = documentsDirectory_blisslink.appendingPathComponent(mediaUrl_blisslink)
        if let image_blisslink = UIImage(contentsOfFile: fileURL_blisslink.path) {
            return image_blisslink
        }
    ...
    private func handleBack_blisslink() {
        ...
        // 检查是否是全屏模式
        if router_blisslink.presentedFullScreen_blisslink != nil {
