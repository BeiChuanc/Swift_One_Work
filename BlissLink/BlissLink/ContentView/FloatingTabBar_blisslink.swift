...
/// 悬浮底部导航栏视图
struct FloatingTabBar_blisslink: View {
    
    // MARK: - 属性
    
    /// 当前选中的标签索引
    @Binding var selectedTab_blisslink: Int
    
    /// 标签选中回调
    var onTabSelected_blisslink: (Int) -> Void
    
    /// 中间按钮的旋转角度
    @State private var centerButtonRotation_blisslink: Double = 0
    ...
    var body: some View {
        ...
        .padding(.top, 12.h_blisslink)
        .padding(.bottom, 8.h_blisslink)
        .background(
            RoundedRectangle(cornerRadius: 25.w_blisslink)
    ...
    private func tabButton_blisslink(
        ...
        let isSelected_blisslink = selectedTab_blisslink == index_blisslink
        ...
        Button(action: {
            ...
            VStack(spacing: 6.h_blisslink) {
                ...
                if isSelected_blisslink {
                    Capsule()
                        .fill(Color.white)
                        .frame(width: 26.w_blisslink, height: 3.h_blisslink)
            ...
            .frame(width: 60.w_blisslink, height: 50.h_blisslink)
        }
    ...
    private var centerButton_blisslink: some View {
        let isSelected_blisslink = selectedTab_blisslink == 2
    ...
    private func handleTabTap_blisslink(index_blisslink: Int) {
        ...
        // 触发选中回调
        onTabSelected_blisslink(index_blisslink)
